...
export class TrashNotesComponent implements OnInit {
  ...
  */
  ...
  get_trash_notes() {
    let arr_id = this.noteOrderService.get_order_list(3);
    this.notes = [];
    let note;
...
deletePermanently(order_num: number) {
  let id = this.notes[order_num]['id'];
  this.noteOrderService.delete(id);
  this.noteStorageService.delete_note(id);
  this.deleteNote(order_num);
}
